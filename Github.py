    ...
    def __init__(self, master):
        ...
        for date in dates:
            ...
            for task_name, task_finished, task_date in tasks:
                ...
                task_time_pretty = "{}:{}".format(
                    task_time_pieces[0], task_time_pieces[1])
                tree.insert("", tk.END, values=(
                    task_name, task_finished_text, task_time_pretty))
...
class Timer(tk.Tk):
    def __init__(self):
        ...
        style.configure("TLabel", foreground="black",
                        background="lightgrey", font=(None, 16), anchor="center")
    ...
    def start(self):
        ...
        if self.task_is_duplicate():
            msg.showerror("Task Duplicate",
                          "Please enter a different task name")
            return
    ...
    def pause(self):
        ...
        if self.worker.paused:
            ...
            self.worker.end_time = self.worker.end_time + \
                datetime.timedelta(seconds=end_timedelta.seconds)
